<template>
  <div class="mybusinessContainer"> 
      <bsnheader :bsn-title="'房速贷管理'" :searchFn="searchFn" :businessType="businessType" :addFun="goToAdd" :isLoading="isLoading" ref="header"></bsnheader>
...
export default {
  ...
  methods: {
    ...
    getData (cb, isQuiet = false) {
         this.api.HouseBusiness.HouseBusinessApi_GetHouseBusinessList(this.where, isQuiet).then(Result => {
    ...
    goToAdd () {
      this.$router.push('/houseBusiness/addhouseBusiness')
    }
  },
  beforeMount () {
    // 获取车易贷业务列表
    this.api.HouseBusiness.HouseBusinessApi_GetSearchConditions().then(Result => {
